...
namespace LowLevelNetworking
{
	...
	public class NetServer : NetHost
	{
		...
		public NetServer(int maxConnections, int port, int simMinTimeout = 0, int simMaxTimeout = 0)
		{
			...
			HostTopology ht = new HostTopology(NetManager.connectionConfig, maxConnections);
			...
			if(!NetHelper.IsSocketValid(socket))
			{
				Debug.Log("NetServer::NetServer( " + maxConnections + " , " + port.ToString() + " ) returned an invalid socket ( " + socket.ToString() + " )");
			}
		...
		public bool SendStream(byte[] buffer, int size, int connId, int channel)
		{
			...
			NetworkTransport.Send(socket, connId, channel, buffer, size, out error);
			if(NetHelper.IsNetworkError(error))
			{
				Debug.Log("NetServer::SendStream( " + buffer.ToString() + " , " + size.ToString() + " ) Failed with reason '" + NetHelper.GetNetworkError(error) + "'.");
				return false;
			}
		...
		public bool DisconnectClient(int connId)
		{
			...
			byte error;
			NetworkTransport.Disconnect(socket, connId, out error);

			if(NetHelper.IsNetworkError(error))
			{
				Debug.Log("NetServer::DisconnectClient( " + connId + " ) Failed with reason '" + NetHelper.GetNetworkError(error) + "'.");
				return false;
			}
		...
		{
			if(!clients.Exists(element => element == connId))
			{
		...
		{
			return clients.Exists(element => element == connId);
		}
	}
}